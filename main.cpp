...
public:
    ...
    void inorderTraversal(Node* node, std::vector<std::string>& result) {
        if (node != nullptr) {
            inorderTraversal(node->left, result);
            result.push_back(node->gatorID);
...
//        removeInorderN(root, count, n +1); // Adjust n to be 0-based index
        std::vector<std::string> inorderResult;
        inorderTraversal(root, inorderResult);

        std::string gatorID = inorderResult[n];
        remove(root, gatorID);
    }
